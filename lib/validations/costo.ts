...
export const CATEGORIA_COSTO_LABELS = {
  ...
  // Agregadas por pedido de Oldemar (feedback sobre el módulo ya entregado).
  REPUESTOS: "Repuestos",
  LLANTAS: "Llantas",
  OTRO: "Otro",
} as const;

export const costoOperativoSchema = z
  .object({
    ...
    // Solo aplica (y se exige, ver refine abajo) cuando categoria = OTRO —
    // el texto específico que escribe el usuario (ej. "Seguros", "Multas"),
    // para poder filtrarlo después como si fuera una categoría más.
    categoriaOtroDetalle: z.string().trim().optional().or(z.literal("")),
  ...
  .refine((datos) => datos.categoria !== "OTRO" || Boolean(datos.categoriaOtroDetalle), {
    message: "Escribí a qué categoría corresponde",
    path: ["categoriaOtroDetalle"],
  });
